    ...
    try:
      ...
      for key in [
        ...
        if (v := normalized_config.get(key)) is not None:
          api_params[key] = v
      #print(str(time.time()-1766990000).split('.')[0], messages)
      response = self._client.chat.completions.create(**api_params)

      # Extract the response text using the v1.x response format
      output_text = response.choices[0].message.content
      print('z',str(time.time()-1766990000).split('.')[0],output_text)
    ...
    if len(batch_prompts) > 1 and self.max_workers > 1:
      with concurrent.futures.ThreadPoolExecutor(
        ...
        for future in concurrent.futures.as_completed(future_to_index):
          ...
          try:
            #print(str(time.time()-1766990000).split('.')[0], batch_prompts[index])
            results[index] = future.result()
            #print('x',str(time.time()-1766990000).split('.')[0],results[index])
          except Exception as e:
            raise exceptions.InferenceRuntimeError(
