...
from .core import format_handler as fh
from .core import tokenizer as tokenizer_lib
...
class Annotator:
  ...
  def annotate_documents(
    ...
    #print('extraction_passes',extraction_passes)
      ...
      for batch in batch_iter:
        ...
        if show_progress:
          ...
          try:
            ...
            pass
        #print(str(time.time()-1766990000).split('.')[0],prompts)
        outputs = self._language_model.infer(batch_prompts=prompts, **kwargs)
        #print(str(time.time()-1766990000).split('.')[0],outputs)
        ...
        for text_chunk, scored_outputs in zip(batch, outputs):
          ...
          #print('align time start',str(time.time()-1766990000).split('.')[0])
          ...
          #print('align time end',str(time.time()-1766990000).split('.')[0])
          for extraction in aligned_extractions:
            #print(str(time.time()-1766990000).split('.')[0],extraction)
            per_doc[text_chunk.document_id].append(extraction)
          #print('align time end2',str(time.time()-1766990000).split('.')[0])
          if show_progress and text_chunk.char_interval is not None:
            chars_processed += (
                text_chunk.char_interval.end_pos
                - text_chunk.char_interval.start_pos
            )
          #print('emit time start',str(time.time()-1766990000).split('.')[0])
